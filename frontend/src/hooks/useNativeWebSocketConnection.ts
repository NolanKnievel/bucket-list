...
import {
  useNativeWebSocket,
  ConnectionState,
} from "../contexts/NativeWebSocketContext";
import { BucketListItem, Member } from "../types";

interface UseNativeWebSocketConnectionProps {
  groupId: string;
  memberId: string;
...
interface UseNativeWebSocketConnectionReturn {
...
/**
 * Custom hook for managing native WebSocket connection to a specific group
 * Handles automatic connection, reconnection, and provides convenient methods
 */
export const useNativeWebSocketConnection = ({
  groupId,
  memberId,
  onMemberJoined,
  onItemAdded,
  onItemUpdated,
  onError,
}: UseNativeWebSocketConnectionProps): UseNativeWebSocketConnectionReturn => {
  const {
    connectionState,
    isConnected,
    isOnline,
